"""  
    author: Shawn
    time  : 11/7/18 5:11 PM
    desc  :
    update: Shawn 11/7/18 5:11 PM      
"""
...
def get_next_batch(batch_size=128):
    ...
    batch_y = np.zeros([batch_size, CAPTCHA_LENGTH * CHARACTERS_LENGTH])
    ...
    for i in range(batch_size):
        ...
        batch_y[i, :] = text_util.text2vec(text)
...
Y = tf.placeholder(tf.float32, [None, CAPTCHA_LENGTH * CHARACTERS_LENGTH])
...
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    w_out = tf.Variable(w_alpha * tf.random_normal([1024, CAPTCHA_LENGTH * CHARACTERS_LENGTH]))
    b_out = tf.Variable(b_alpha * tf.random_normal([CAPTCHA_LENGTH * CHARACTERS_LENGTH]))
...
def train_through_cnn():
    ...
    predict = tf.reshape(output, [-1, CAPTCHA_LENGTH, CHARACTERS_LENGTH])
    max_idx_p = tf.argmax(predict, 2)
    max_idx_l = tf.argmax(tf.reshape(Y, [-1, CAPTCHA_LENGTH, CHARACTERS_LENGTH]), 2)
